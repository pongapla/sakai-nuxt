                        ...
                        >
                            ...
                            <Column style="width: 300px; text-align: center">
                                ...
                                <template #body="slotProps">
                                    <div>
                                        <button
                                            v-for="(lang, index) in slotProps.data.languageList"
                                            :key="index"
                            ...
                            <Column field="action" style="width: 150px; text-align: center">
                                ...
                                <template #body="slotProps">
                                    <div class="flex justify-content-center">
                                        ...
                                        <Button icon="pi pi-pencil" outlined rounded class="mr-1" @click="editCategory(slotProps.data)" />
            ...
            <Dialog :header="dialogHeader" v-model:visible="display" :breakpoints="{ '960px': '70vw' }" :style="{ width: '30vw', height: '40vh' }" :modal="true" @hide="closeDialog">
                ...
                <div style="margin-left: 20px">
                    <div class="grid align-items-center mt-2" style="display: flex; align-items: center">
                        ...
                        <div class="col mb-3">
                            <Dropdown :options="languages" v-model="selectedLangauge" optionLabel="lang_name" placeholder="Select an language" optionValue="id" class="custom-input" />
                        </div>
                    </div>
                </div>
...
const search = async () => {
    ...
    try {
        if (categorySearchQuery.value === '') {
            ...
            const filteredCategoryInfo = categoryInfo.value.filter((category) => category.title.toLowerCase().includes(categorySearchQuery.value.toLowerCase()));
...
const save = () => {
    ...
    if (formData.group) {
        isEditMode.value ? updateCategory(formDataObject) : addCategoryLanguage(formDataObject);
    } else {
...
const updateCategory = async (formDataObject: FormData) => {
    try {
        const result = await catergoriesStore.updateCategoryLanguage(formDataObject);
        ...
        const index = categoryInfo.value.findIndex((item) => item.group === newUpdate.group);

        if (index !== -1) {
            categoryInfo.value[index] = newUpdate;
        } else {
            categoryInfo.value.push(newUpdate);
        }
...
const editCategory = (data: any) => {
    ...
    formData.group = data.group;
    dialogHeader.value = 'Edit Category Language';
...
const deleteSelectedCategory = async () => {
    ...
    try {
        ...
        const titleSeach = selectedCategory.value.title;
...
const filteredCategoryInfo = computed(() => {
    if (categorySearchQuery.value === '') {
        ...
        return categoryInfo.value.filter((category) => category.title.toLowerCase().includes(categorySearchQuery.value.toLowerCase()) || category.group.toLowerCase().includes(categorySearchQuery.value.toLowerCase()));
    }
});
